...
### 제외 (Out-of-scope)
- 정형 Oracle DB 등 내부 시스템 연계 (본 구축 2단계)
- 완전 자동 적재 파이프라인 (수동 재실행 CLI 스크립트만 제공)
...
| 유형 | 내용 |
|------|------|
| 응답 시간 | 챗봇 평균 5초 이내 목표 (스트리밍으로 first-token 체감 단축) |
| 비용 통제 | 토큰 사용량 로깅, config 기반 일별 한도 경고 |
| 가드레일 | 프롬프트 인젝션 필터 · 근거 없을 시 거부 · 민감정보(PoC는 개인정보 미포함) |
| 관측 | 구조화 로그 + (선택) OpenTelemetry → Foundry tracing |
| 보안 | 키/엔드포인트는 `.env`/시크릿, 코드 커밋 금지 |
