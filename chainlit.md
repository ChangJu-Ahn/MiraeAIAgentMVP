...
## 에이전트가 하는 일
질문을 하위 질의로 분해해 여러 인덱스를 조회하고(**agentic retrieval**), 결과가 부족하면 질의를 바꿔 **다시 조회**하며, 근거를 인용해 답변합니다. 자료에 없으면 답변을 거부합니다(**할루시네이션 방어**). 추론(계량 모델)·도구 실행 과정이 단계로 표시되고, 답변은 토큰 단위로 스트리밍됩니다.
...
- **Agent Framework** — 오케스트레이션, 미들웨어 트레이스, App Insights(OpenTelemetry)
